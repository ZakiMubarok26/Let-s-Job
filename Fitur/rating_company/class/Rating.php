...
class Rating{
	...
	private function getData($sqlQuery) {
		...
		if(!$result){
			die('Error in query: '. mysqli_error($this->dbConnect)); // Menambahkan koneksi sebagai parameter
		}
		$data = array();
		while($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) { // Gunakan $result bukan $query
			$data[] = $row;            
		}
		return $data;
	}
